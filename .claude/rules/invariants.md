...
| 8 | Every tenant-scoped table carries `org_id` and an RLS policy. `app.current_org` is set per connection; application-level filtering is never the only defence. | ADR-010 | The generated RLS negative matrix §5.3 — for every table, a session scoped to org A reads, updates and deletes zero org B rows. **It enumerates the Drizzle schema, so it sees declared tables and nothing else**; the completeness check in migration 0002 counts what PostgreSQL counts, and the two are not interchangeable — see the note below | M0 |
...
**Invariant 8 has two guards, and only one of them can see a partition, 2026-09-21.** The
generated matrix in `packages/db/tests/rls.test.ts` derives its table list from the Drizzle schema,
which is the right source for "did somebody add a tenant table and forget the policy" and is blind
to every table created in SQL rather than declared in TypeScript. The monthly partitions of
`session_events` and `proctor_events` are exactly that: their names do not exist until
`infra/postgres/init/04-partitions.sql` runs, and **a partition does not inherit its parent's
row-level security** — PostgreSQL applies the parent's policies to rows reached through the parent
and the partition's own when the partition is named directly. Each was therefore a way round tenant
isolation for anyone who could write a name generated by a fixed rule, and the matrix could not
have found it. The completeness check in migration 0002 did, because it asks `pg_class` rather than
a TypeScript list. Keep both: one catches the declared table nobody policed, the other catches the
table nobody declared. docs/14 T-043 has the measurements and
[`docs/17-engineering-standards.md`](../../docs/17-engineering-standards.md) §7b the lesson.
