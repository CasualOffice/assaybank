...
When translations arrive, a translated version is a distinct `question_version` linked to its source by a translation-group identifier. It accumulates its own statistics from n = 0. It does not inherit the source question's p-value, discrimination or exposure count.
