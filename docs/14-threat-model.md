...
*Action.* `H-149` (proposed) — session cookies are `HttpOnly`, `Secure`, `SameSite=Lax`, host-prefixed, with the identifier regenerated on login, on privilege change and on logout; explicit tests for fixation, for logout invalidating server-side, and for the candidate and staff cookie namespaces being disjoint.
...
*Action.* `H-153` (proposed) — double-submit CSRF tokens on every state-changing staff route, an `Origin`/`Sec-Fetch-Site` check rejecting cross-site mutations, CORS configured from `CORS_ALLOWED_ORIGINS` with credentials allowed only for exactly listed origins and never `*`, and a test that a cross-origin mutation is rejected with `forbidden`.
