...
**About task references.** The highest allocated id in [`../project/TRACKER.md`](../project/TRACKER.md) at the time of writing is `H-109`. Existing tasks are cited where one already covers the work. Ids `H-110` and above are **proposed here and not yet in the tracker**; when they are added, they must be allocated in ascending order per the tracker's id allocation rule, and this document updated if the allocation differs. A proposed id is marked `(proposed)`.
...
*Action.* `H-110` (proposed) — exec nodes run from an immutable image with a read-only root filesystem, a per-run tmpfs work directory, and recycling after a bounded number of runs; document the recycle interval in [`infra/README.md`](../infra/README.md).
...
*Action.* Covered by `H-111` (proposed); additionally `H-112` (proposed) — a standing regression suite of hostile submissions, one per supported language, that attempts DNS resolution, outbound TCP and outbound HTTP, and is asserted to fail. Feeds [`06-testing-strategy.md`](06-testing-strategy.md).
...
*Action.* `H-113` (proposed) — a hostile-submission suite covering fork bombs, allocation bombs, CPU spin, sleep-forever and zip-bomb-style expansion, asserting that each is killed inside the configured wall-time budget and that the exec node returns to service; plus the sandbox-timeout-rate metric and alert from [`02-HLD.md`](02-HLD.md) §8, tracked as `H-079`.
...
*Action.* `H-114` (proposed) — enforce `EXEC_MAX_OUTPUT_BYTES` in `packages/exec-adapter` with the truncation marked in the result, and a second hard truncation in `packages/grading` before any row is written; test asserts a 1 GB stdout submission produces a bounded row and a `passed = false` result rather than an error.
...
*Action.* `H-116` (proposed) — the candidate-facing result serialiser is the only path to a candidate, and a contract test asserts that for a submission with a deliberately distinctive hidden expectation (a UUID as expected stdout), that string never appears anywhere in any candidate-visible response body or SSE frame. Extends the serialisation test required by [`02-HLD.md`](02-HLD.md) §7.
...
*Action.* `H-117` (proposed) — an adapter-level test asserting the `execute()` payload contains no expected output, no question identifier and no organisation identifier; plus a note in [`04-ADRs.md`](04-ADRs.md) terms if the checker placement is ever revisited.
...
*Action.* `H-118` (proposed) — checker code and SQL fixtures execute inside the same sandbox as candidate submissions, through the same adapter, with the same limits and no egress; the worker supplies inputs and reads a verdict. Import never sets `checker_code` or `fixture_sql` from an external file — those fields are authored in-product only, and the import job rejects rows carrying them. M2 scope, blocks the `custom_checker` grading mode shipping.
...
*Action.* `H-122` (proposed) — ticket redemption is a single atomic Valkey operation; the collab server rejects a second connection for the same participant identity and writes a `session_events` row when it does, so the interviewer sees the attempt.
...
*Action.* `H-123` (proposed) — session cookies are `HttpOnly`, `Secure`, `SameSite=Lax`, host-prefixed, with the identifier regenerated on login, on privilege change and on logout; explicit tests for fixation, for logout invalidating server-side, and for the candidate and staff cookie namespaces being disjoint.
...
*Action.* `H-125` (proposed) — bulk export is asynchronous, audited, notifies the org's admins on completion, and is rate-limited separately from the general staff budget; volume anomaly detections per §9. `H-126` (proposed) — support enforced MFA or IdP-only login per org, configurable in `/org/settings`.
...
*Action.* `H-127` (proposed) — double-submit CSRF tokens on every state-changing staff route, an `Origin`/`Sec-Fetch-Site` check rejecting cross-site mutations, CORS configured from `CORS_ALLOWED_ORIGINS` with credentials allowed only for exactly listed origins and never `*`, and a test that a cross-origin mutation is rejected with `forbidden`.
...
*Action.* `H-128` (proposed) — every candidate-facing route resolves the target row through the token's attempt id in the same query (`... JOIN attempt_questions aq ON aq.id = $1 AND aq.attempt_id = $2`), never by fetching then checking; an authorisation matrix test enumerates each candidate route against an id belonging to another attempt and asserts `not_found`, never `forbidden`, so the response is not an existence oracle.
...
*Action.* `H-129` (proposed) — the job role is granted per-table, write-only where possible, and every job that touches tenant data must either set `app.current_org` for the duration (preferred, keeping RLS active) or be on an explicit allow list of cross-tenant jobs with a documented justification. A lint rule fails CI on a raw `DATABASE_JOB_ROLE` connection outside `apps/worker`. `H-130` (proposed) — extend the `H-017` negative-test suite to cover the job role: a job processing a submission from org A must not read or write any org B row, asserted per job type.
...
*Action.* `H-131` (proposed) — all filter and sort parameters are zod enums in `packages/contracts` resolving to a fixed allow list of column names; raw SQL fragments are permitted only through Drizzle's `sql` template with bound parameters, enforced by a lint rule; cursors are opaque, signed and validated before decoding.
...
*Action.* `H-133` (proposed) — every request schema in `packages/contracts` is `.strict()`, rejecting unknown keys with `validation_failed`; `.passthrough()` is banned by lint; the database update is constructed from an explicit field list, never from the parsed body by spread. A test posts every scoring and status column to every candidate-facing `PATCH` and asserts rejection.
...
*Action.* `H-136` (proposed) — an integration test that submits with a client clock skewed by ±6 hours and asserts identical server behaviour; the candidate UI re-syncs its countdown from `server_time` on every response and shows a reconnecting state rather than a frozen timer when heartbeats fail.
...
*Action.* `H-137` (proposed) — autosave carries a monotonic client sequence number per answer; the server applies a write only if the sequence exceeds the stored one, returning `conflict` otherwise, and the applied sequence is returned so the client can detect divergence. Rejected out-of-order writes increment a metric, since a spike is either a network problem or a replay attempt.
...
*Action.* Covered by `H-144` (proposed) — the signed timestamp plus a documented tolerance window; [`09-ats-integration.md`](09-ats-integration.md) states the `event_id` deduplication requirement and the tolerance as consumer obligations, with a reference verification snippet.
...
*Likelihood* medium · *Impact* high · *Residual* medium.
*Existing control.* Thin. `POST /questions/import` is asynchronous and reports per-row errors ([`03-API-spec.md`](03-API-spec.md) §4) so there is a hook for validation, and `question.publish` is a separate permission with human review. React escapes by default — but markdown rendering exists precisely to bypass that escaping, which is the point of the threat.
*Residual reasoning.* **This is the most underrated path in the register.** Authors are trusted, so question content feels like trusted content — and then the import endpoint accepts thousands of prompts from sources nobody has read. The trust attached to the *author* does not transfer to the *content*.
*Action.* `H-148` (proposed) — markdown rendering uses a sanitising pipeline with an allow list of elements and attributes, raw HTML disabled, `javascript:` and `data:` URLs rejected, and code blocks rendered as text; a strict Content-Security-Policy on both apps with no `unsafe-inline` and no `unsafe-eval` (noting Monaco's worker requirements must be satisfied with hashes or nonces, not a blanket exemption); the import job rejects rows whose prompt contains raw HTML rather than silently stripping it, so the author sees what was refused; a test corpus of XSS payloads runs against the renderer in CI. Applies equally to `scorecards.notes_md` and any other markdown field.
...
*Action.* `H-149` (proposed) — a single CSV writer in `packages/core-domain` used by every export path, prefixing any field beginning with `=`, `+`, `-`, `@`, tab or carriage return with a single quote, quoting all fields, and writing UTF-8 with a BOM; a test asserts the payloads above round-trip inert. Applies to candidate exports, question exports and the audit-log export alike.
...
| XSS corpus | A payload corpus rendered through the markdown pipeline in both apps, with CSP asserted | T-038 | Every build |
