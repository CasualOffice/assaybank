...
- [x] Staff console shell and the question authoring UI including the test-case editor — 2026-09-20, `H-037` and `H-038`: sidebar shell with grouped navigation from the route manifest, the question list, and the authoring editor with kind-aware content editors, a markdown preview and an irreversible publish. axe-clean in every state. The console's auth guard landed 2026-09-20 (`H-177`): nothing renders for a visitor the console cannot identify, and an `unauthenticated` from any query returns to sign-in. The server half of that track — session cookie attributes (`H-149`), CSRF (`H-153`), OIDC callback validation (`H-150`) — is not built
