...
| **Overall RAG** | amber |
| **Build** | green — CI, Security, Docs and Licences all passing on `main` since 2026-09-17, the first green run; **2,944 tests**, 0 failing, 0 skipped — counted, not estimated (`turbo run test --force`, plus the leak and fixtures projects) |
...
**Application code now exists.** 75 of 196 backlog tasks are done, counted from the tracker rows. The jump from 177 rows is the guided flow — `H-178` to `H-189`, from reading the console back as a product rather than as a set of screens ([`../docs/18-hiring-workflows.md`](../docs/18-hiring-workflows.md)).
...
The first stage landed: composing an assessment from a role (`H-196`). `composeFromRole` turns a role's required skills, weights and bands into section rules — proportional by largest remainder so the rules sum to exactly the paper asked for, with a floor of one question per required skill so the lightest are not rounded out of existence. `GET /job-roles/{id}/assessment-plan` reports it with the bank's in-band count against every rule, and `POST /assessments/auto` refuses to save a paper the bank cannot fill rather than letting it fail for the first candidate to open the link. The console screen landed with it (`H-179`): a role's **Compose an assessment** is a link rather than a promise, and it opens on the composed paper rather than an empty form — because the role has already made the four judgements a wizard would ask for. Where the bank falls short it names the skills, does the subtraction, and refuses the save; where it does not, saving lands on `/assessments` with the new row marked. Stage three of docs/18's seven is done, and stage four — invitations — is the next one with no code. The console's first screen is no longer a phase placeholder: `/` now leads with one sentence saying what to do next — decided from role coverage and the review queue, by a rule table with a test per rule — and carries the two panels that are its evidence (`H-191`, docs/18 §1). Nothing on it scores, orders or recommends a candidate, and it says so on the screen.
