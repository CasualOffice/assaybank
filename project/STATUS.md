...
| **Overall RAG** | amber |
| **Build** | green — CI, Security, Docs and Licences all passing on `main` since 2026-09-17, the first green run; **2,896 tests**, 0 failing, 0 skipped — counted, not estimated (`turbo run test --force`, plus the leak and fixtures projects) |
...
**Application code now exists.** 73 of 195 backlog tasks are done, counted from the tracker rows. The jump from 177 rows is the guided flow — `H-178` to `H-189`, from reading the console back as a product rather than as a set of screens ([`../docs/18-hiring-workflows.md`](../docs/18-hiring-workflows.md)).
