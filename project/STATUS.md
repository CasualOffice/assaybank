...
| **Overall RAG** | amber |
| **Build** | green — CI, Security, Docs and Licences all passing on `main` since 2026-09-17, the first green run; **2,967 tests**, 0 failing, 0 skipped — counted, not estimated (`turbo run test --force`, plus the leak and fixtures projects) |
...
**Application code now exists.** 76 of 196 backlog tasks are done, counted from the tracker rows. The jump from 177 rows is the guided flow — `H-178` to `H-189`, from reading the console back as a product rather than as a set of screens ([`../docs/18-hiring-workflows.md`](../docs/18-hiring-workflows.md)).
...
P2 question bank: import/export (the M0 exit criterion) and the authoring console. The console now reads and writes the bank end to end — list, filter, edit, preview, send for review, publish — and author markdown is parsed to nodes rather than sanitised as HTML (ADR-022), which is the T-038 mitigation and the reason a prompt containing `<script>` renders as text. Both front ends carry a `default-src 'none'` Content-Security-Policy with no `unsafe-inline` and no `unsafe-eval`. The console also has its first screen that is about hiring rather than about the bank: `/roles` answers, per role, whether the bank can measure it — blocked, thin or ready — from an endpoint that was already built and had never been asked. What it still lacks is its auth guard (`H-177`). The two file formats are built and round-trip every kind exactly — the JSON bank document with full history and attribution, the QTI 2.1 package for the served version — and a bank now goes out of one organisation and into an empty one through PostgreSQL with nothing lost, export → import → export byte-identical. Import and export are reachable over HTTP as jobs — the request commits a row and its audit record together, a relay in the worker claims committed rows, and an import resumes at a per-item checkpoint if retried (ADR-021). All four dataset importers are built — HumanEval, MBPP, LBPP and Exercism, importing as drafts graded by unit tests (ADR-024), with each dataset's licence taken from docs/05 §2 rather than from whoever uploads the file, and an attributions page discharging the CC-BY credit the import creates. **M0's exit criterion is met** (2026-09-20, `H-040`): 200 questions loaded through the real dataset reader and import job, tagged across a six-skill taxonomy to three overlapping job roles, exported, re-imported into an empty organisation and compared — as a test that runs on every build, so the criterion is either true now or red. What remains in M0 is scope rather than the bar, and the console's auth guard landed with it (`H-177`) — nothing renders for a visitor it cannot identify. Two-thirds of the server half of that track landed the same day: the staff session cookie is `__Host-` prefixed rather than `__Secure-` (`H-149`), which confines it to exactly the API's host instead of merely promising it was set over https, and every state-changing staff request now carries a signed double-submit CSRF token bound to its session as well as passing the origin check (`H-153`). The two are one control: a double-submit token a sibling subdomain could write would prove nothing, and the prefix is what stops it. OIDC callback validation (`H-150`) closed on 2026-09-21, and closing it found that the OIDC flow had never worked at all: `POST /auth/oidc/start` discarded the signed `state` cookie Better Auth sets there, so every callback was refused before it looked at the assertion it carried. The existing tests could not see it, because they all asserted refusals and a refusal is what they got. The whole staff-session track is now built.

**The thin path through docs/18 has started (2026-09-21).** Five of its seven stages did not exist, and the survey that preceded the work found why that is less daunting than it reads: the hard parts are built and unreachable. `packages/core-domain` already holds the attempt state machine, the draw, the option shuffle, deadlines and the per-skill roll-up; `packages/grading` holds test-case comparison, MCQ and short-answer marking and weighted aggregation — 444 tests of domain logic with no HTTP route in front of any of it. What is missing between a role and a report is mostly wiring and screens, not invention.

The first stage landed: composing an assessment from a role (`H-196`). `composeFromRole` turns a role's required skills, weights and bands into section rules — proportional by largest remainder so the rules sum to exactly the paper asked for, with a floor of one question per required skill so the lightest are not rounded out of existence. `GET /job-roles/{id}/assessment-plan` reports it with the bank's in-band count against every rule, and `POST /assessments/auto` refuses to save a paper the bank cannot fill rather than letting it fail for the first candidate to open the link. The console screen landed with it (`H-179`): a role's **Compose an assessment** is a link rather than a promise, and it opens on the composed paper rather than an empty form — because the role has already made the four judgements a wizard would ask for. Where the bank falls short it names the skills, does the subtraction, and refuses the save; where it does not, saving lands on `/assessments` with the new row marked. Stage four landed the same day (`H-182`): publish an assessment, paste a list of addresses in whatever shape it was copied in, and get one single-use link per candidate — served exactly once, because only a peppered hash is stored. An address already holding a live link is skipped rather than given a second sitting. Walked end to end against a real API, including a candidate redeeming a link into an attempt and being refused a second time.

That leaves the candidate's own half. `apps/candidate` has its shell, its countdown and its reconnect banner, and `/attempt` is still a placeholder — so a link now works right up to the point where somebody would answer a question. Sending the invitation by email (`H-189`) is not built either; the console shows the links to send. The console's first screen is no longer a phase placeholder: `/` now leads with one sentence saying what to do next — decided from role coverage and the review queue, by a rule table with a test per rule — and carries the two panels that are its evidence (`H-191`, docs/18 §1). Nothing on it scores, orders or recommends a candidate, and it says so on the screen.

Running the documented local path on a clean machine, `make up && make migrate`, found three defects that every test was green through, because nothing in CI runs container init (`H-192`, docs/14 T-043). Two were tenant-isolation holes in the database a developer actually works against: `bank_jobs` had no policy in the bootstrap, and the monthly partitions of `session_events` and `proctor_events` had no row-level security of their own — which a partition does not inherit, so naming one directly went round the parent's policy. The migrations' own completeness check is what caught both. A static check now guards the two shapes that can be guarded without a database.
