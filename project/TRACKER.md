...
| H-150 | P0 | OIDC callback validates issuer, audience, `nonce`, `exp`, `iat` skew and the PKCE verifier with `state` bound to the originating session; email claims map to an existing user, never auto-provision — _done 2026-09-21: proven by an identity provider that signs real RS256 assertions and can be wrong on purpose. Building it found the flow had never worked — the start route discarded Better Auth's signed `state` cookie, so every callback was refused before any assertion was read, and a file of negative tests passed throughout (docs/17 §8a). Two downgrades closed: no provider at all when discovery cannot support verification, and a refusal when a token exchange returns no assertion. `iat` skew is not separately enforced — see T-015_ | auth | M0 | — | T-015 | M | done | _unassigned_ |
...
| H-179 | P1 | Compose an assessment from a role rather than from the bank, **in the console**: the screen that shows the plan, names the skills the bank falls short on, and saves it. The derivation and the feasibility check are `H-196` and are built — _done 2026-09-21: `/roles/{id}/compose` opens with the composed paper rather than an empty form, names the skills the bank falls short on with the arithmetic done, refuses the save while it is infeasible, and lands on `/assessments` with the new row marked. The Roles screen's button is a link rather than a promise_ | web | M1 | H-178, H-196 | docs/18 §2.2, FR-6 | L | done | _unassigned_ |
...
| H-182 | P1 | Invitations sent from the console: bulk from a pasted list, a CSV or an ATS pull, with a per-invitation delivery state a recruiter can see and a resend that does not mint a second attempt — _done 2026-09-21 for the console half: publish, then paste a list in any shape a recruiter copies it in, and get one single-use link per candidate served exactly once. An address already holding a live link is skipped rather than given a second sitting. Walked end to end against a real API, including a candidate redeeming a link into an attempt. The CSV and ATS pull are `H-186`/`H-187`; **sending the email is `H-189` and is not built** — the console shows the links to send_ | web | M1 | H-179 | docs/18 §2.4, FR-27 | M | done | _unassigned_ |
...
| H-191 | P0 | The dashboard says what to do next rather than showing counters: one sentence decided from role coverage and the review queue, with the two panels under it as its evidence — replacing the phase placeholder that was the first screen after signing in — _done 2026-09-20: `/` reads `GET /job-roles`, each role's coverage and `GET /questions?status=review`, and `dashboard-guidance.ts` picks one of five rules with a test per rule. No server work: every endpoint already existed and none was being asked_ | web | M0 | H-178 | docs/18 §1 | M | done | _unassigned_ |
| H-192 | P0 | The container bootstrap and the migrations cannot drift into less isolation than the schema declares: a static check that every table `infra/postgres/init` creates carries a policy, and that every migration statement is replayable — _done 2026-09-20 (docs/14 T-043): `make up && make migrate` was broken on a clean machine in three ways with every test green. `bank_jobs` had no policy in the bootstrap, and the event partitions had no row-level security of their own, which a partition does not inherit_ | db | M0 | — | T-043, docs/17 §7b | M | done | _unassigned_ |
| H-193 | P1 | A sign-out that does not reach the server says so: the cache is cleared either way, but the session lives on the server and a silent failure leaves somebody signed in on a shared machine believing they are not — _done 2026-09-20, found by pressing the button against a real API_ | web | M0 | H-177 | docs/14 T-014 | S | done | _unassigned_ |
| H-194 | P2 | CI runs the formatting gate it is documented as running: `format:check` existed in `package.json`, no workflow called it, and Prettier drifted on four files while docs/06 described the pull-request stage as running everything repository-wide — _done 2026-09-21_ | infra | M0 | — | docs/06 §CI stages, docs/17 §7b | S | done | _unassigned_ |
| H-195 | P1 | The console shows coverage as figures rather than as sentences, and spends emphasis only on exceptions: a per-skill count and bar on a fixed scale, a one-line guidance band in place of the alert slabs, and a badge only where a row differs from the norm — _done 2026-09-21_ | web | M0 | H-178, H-191 | docs/15 §6, docs/18 §2.1 | M | done | _unassigned_ |
| H-196 | P1 | The server half of composing from a role: `composeFromRole` derives one section rule per required skill from the role's weights and bands, `GET /job-roles/{id}/assessment-plan` reports it with the bank's in-band count per rule, and `POST /assessments/auto` refuses to save a paper the bank cannot fill — _done 2026-09-21. `H-179` is now the console half only_ | api | M1 | H-178 | docs/03 §5, docs/18 §2.2 | M | done | _unassigned_ |
