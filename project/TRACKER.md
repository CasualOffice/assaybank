...
| H-033 | P2 | QTI 2.1 import and export, round-trip without loss across all eight question kinds — _partial 2026-09-17: the QTI 2.1 package codec and its round trip over all eight kinds are done in `apps/worker/src/interchange/`; the import job, the routes and the round trip through a real database remain_ | worker | M0 | H-032 | M0 exit | L | todo | _unassigned_ |
| H-034 | P2 | JSON bank export in an open, documented shape, with CC-BY attribution preserved in the payload — _partial 2026-09-17: the JSON bank document codec, with attribution in the header and round-trip tests, is done; the export job and route remain_ | worker | M0 | H-032 | FR-29, G6 | M | todo | _unassigned_ |
