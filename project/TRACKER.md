...
| H-179 | P1 | Compose an assessment from a role rather than from the bank, **in the console**: the screen that shows the plan, names the skills the bank falls short on, and saves it. The derivation and the feasibility check are `H-196` and are built | web | M1 | H-178, H-196 | docs/18 §2.2, FR-6 | L | todo | _unassigned_ |
