...
CMD ["pnpm", "--filter", "@hiring/worker", "dev"]

FROM deps AS build
ENV NODE_ENV=production
COPY . .
RUN pnpm turbo run build --filter @hiring/worker...
RUN pnpm --filter @hiring/worker --prod deploy --legacy /deploy
